...
import { getFriendRelationState, getUserProfile } from "@/lib/firebase";
...
export default function PublicProfilePage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (active) toast.error("Profile load failed");
      } finally {
  ...
  if (loading) {
    return <div className="glass-card p-6 text-center text-gray-400">Loading profile...</div>;
  }
    ...
    <div className="space-y-5 max-w-2xl mx-auto animate-card-in">
      ...
      {!own && (
        <div className="grid grid-cols-2 gap-3">
          ...
          <Button variant="gold" onClick={() => navigate(`/leaderboard`)} className="min-h-[50px]" leftIcon={<Swords className="w-4 h-4" />}>Challenge</Button>
        </div>
      )}
    </div>
  );
